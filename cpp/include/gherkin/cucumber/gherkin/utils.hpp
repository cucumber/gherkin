...
namespace cucumber::gherkin
{
    ...
    namespace detail
    {
        ...
        inline char32_t utf8_next(utf8_ptr& p, utf8_ptr end)
        {
            ...
            if ((*p & 0xF0) == 0xE0 && (end - p) >= 3 && (p[1] & 0xC0) == 0x80 && (p[2] & 0xC0) == 0x80)
            {
            ...
            if ((*p & 0xF8) == 0xF0 && (end - p) >= 4 && (p[1] & 0xC0) == 0x80 && (p[2] & 0xC0) == 0x80 && (p[3] & 0xC0) == 0x80)
            {
    ...
    struct re_patterns
    {
        static auto convert(std::string_view sv)
        {
    ...
    template<typename CharT>
    std::basic_string_view<CharT> as_view(const std::basic_string<CharT>& s)
    {
    ...
    template<typename CharT>
    std::basic_string<CharT> strip(std::basic_string_view<CharT> what, const strip_pattern<CharT>& p)
    {
        std::basic_regex<CharT> re(p.str());
        std::basic_string<CharT> empty;
        std::basic_string<CharT> result;

        std::regex_replace(std::back_inserter(result), what.begin(), what.end(), re, empty);

        return result;
    }

    template<typename CharT>
    std::basic_string<CharT> lstrip(std::basic_string_view<CharT> in, re_pattern p = re_pattern::all_spaces)
    {
        return strip(in, strip_pattern<CharT>(re_pattern::bol, re_patterns<CharT>::get(p)));
    }

    template<typename CharT>
    std::basic_string<CharT> lstrip(const std::basic_string<CharT>& in, re_pattern p = re_pattern::all_spaces)
    {
        return lstrip(as_view(in), p);
    }

    template<typename CharT>
    std::basic_string<CharT> rstrip(std::basic_string_view<CharT> in, re_pattern p = re_pattern::all_spaces)
    {
        return strip(in, strip_pattern<CharT>(re_patterns<CharT>::get(p), re_pattern::eol));
    }

    template<typename CharT>
    std::basic_string<CharT> rstrip(const std::basic_string<CharT>& in, re_pattern p = re_pattern::all_spaces)
    {
        return rstrip(as_view(in), p);
    }

    template<typename CharT>
    std::basic_string<CharT> strip(std::basic_string_view<CharT> in, re_pattern p = re_pattern::all_spaces)
    {
        return lstrip(rstrip(in, p), p);
    }

    template<typename CharT>
    std::basic_string<CharT> strip(const std::basic_string<CharT>& in, re_pattern p = re_pattern::all_spaces)
    {
